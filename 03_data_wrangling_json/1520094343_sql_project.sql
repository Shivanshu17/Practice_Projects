...
SELECT COUNT(CASE WHEN membercost=0.0 THEN 1 ELSE NULL END) AS num_zero_cost_facd FROM Facilities
...
SELECT facid, name, membercost, guestcost, initialoutlay, monthlymaintenance FROM Facilities WHERE name LIKE '%2'
...
in question. */
SELECT name, monthlymaintenance, CASE WHEN monthlymaintenance>100 THEN 'expensive' ELSE 'cheap' END AS type_of_facility FROM Facilities
...
SELECT Name, Firstname, Surname, cost FROM
	(SELECT Facilities.name AS Name, Members.firstname AS Firstname, Members.surname AS Surname,
    	CASE WHEN Members.memid = 0 THEN Bookings.slots*Facilities.guestcost ELSE Bookings.slots*Facilities.membercost END AS cost
 		FROM Members
			INNER JOIN Bookings ON Members.memid = Bookings.memid
			INNER JOIN Facilities ON Facilities.facid = Bookings.facid
     	WHERE Bookings.starttime>='2012-09-14'
          AND Bookings.starttime<'2012-09-15') AS iteration
	WHERE cost>30
ORDER BY cost DESC
...
SELECT Name, revenue FROM
	(SELECT Facilities.name as Name, SUM(Bookings.slots * CASE WHEN Bookings.memid = 0 THEN Facilities.guestcost ELSE Facilities.membercost END) as revenue
     	FROM Bookings
     		INNER JOIN Facilities ON Bookings.facid = Facilities.facid
